...
int main(int argc, const char** argv)
{
	...
	if (capture.isOpened())
	{
		...
		thread thrd_7(detectAndDraw, cascade, nestedCascade, scale, tryflip);
		thread thrd_8(detectAndDraw, cascade, nestedCascade, scale, tryflip);
		thrd_1.detach();
		thrd_2.detach();
		thrd_3.detach();
		thrd_4.detach();
		...
		while (1)
		{
			...
			for (; a < 1000; )
			{
				...
				for (i = 0; i != faces.size(); ++i)
				{
					...
					Mat imgtem;
					auto x1 = cvRound(faces[i].x*scale), y1 = cvRound(faces[i].y*scale), x2 = cvRound((faces[i].x + faces[i].width - 1)*scale), y2 = cvRound((faces[i].y + faces[i].height - 1)*scale);
					cout << "x1=" << x1 << "\t" << "y1=" << y1 << "\t" << "x2=" << x2 << "\t" << "y2=" << y2 << "\t" << "y2-y1=" << y2 - y1 <<"\t"<< "x2-x1=" << x2 - x1<<"\n" << endl;
